...
class PinjamBus extends Component {
  ...
  renderHeader = () => {
    ...
    const addIcon = (style) => (
      <EvaIcon {...style} name="plus-square-outline"/>
    );
    ...
    return (
      <Layout style={{ flex: 1, padding: 8 }}>
        <Button onPress={() => navigation.navigate('TambahPinjamBusScreen', { peminjamans })} size="large" style={{ flex: 1, marginTop: 8 }} status="primary" icon={addIcon}>PINJAM BUS</Button>
      </Layout>
    );
  }
  ...
  renderItem = ({ item }) => {
    ...
    moment.locale('id');
    const tgl_new = moment(item.tgl_peminjaman).format('LL');
    ...
    return (
      ...
      <Layout style={style.itemContainer} level="2">
        ...
        <Layout style={style.itemContentRight}>
          <Text style={{ flexDirection: 'row', flexWrap: 'wrap' }} category="h6">Acara: {item.nama_acara}</Text>
          <Text style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 2 }} category="p1">Tujuan: {item.tujuan}</Text>
          <Layout style={style.divider} />
          <Text style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 4 }} category="label">Bus: {item.bus.bus}</Text>
          <Text style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 2 }} category="label">No. Polisi: {item.bus.no_polisi}</Text>
          <Text style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 2 }} category="label">Tgl. Pinjam: {tgl_new}</Text>
        </Layout>
      </Layout>
      // </Touchable>
    );
    ...
    return (
      <Layout style={style.container} level="2">
        <List
          ...
          refreshControl={<RefreshControl refreshing={isRefresh} onRefresh={() => this.getPeminjaman('refresh')} tintColor="#FF3300" colors={['#FF3300']} />}
